...
class TranslationController extends CRUDController
{
    ...
    public function createTranslationAction($id, $from_site, $to_site, $fqcn): Response
    {
        ...
        return $this->redirect($editUrl . '?site=' . $site1->getId());
    }
    ...
    {
        $smartServiceConf = $this->parameterBag->get('partitech_sonata_extra.smart_service');

        $proposal = false;
        if (true === $smartServiceConf['seo_proposal_on_article']) {
            $seo_proposal_provider = $smartServiceConf['seo_provider'];

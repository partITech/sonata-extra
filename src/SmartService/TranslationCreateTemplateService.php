...
class TranslationCreateTemplateService
{
    ...
    public function getClassNameByObject($object): string
    {
        return str_replace('\\', '_', get_class($object));
    }

    public function getClassNameByEntityClass($entity): string
    {
        return str_replace('\\', '_', $entity);
    }

    public function createDirectory($projectDir = false): void
    {
    ...
    public function createPayloadArray($payLoad, $translateArray): void
    {
    ...
    public function createPayloadHtmlField($field, $payLoad, $placeholders): void
    {
        if (empty($this->projectDir)) {
            $this->logger->critical("projectDir can not be empty in TranslationCreateTemplateService::createDirectory");
            throw new InvalidArgumentException("projectDir can not be empty in TranslationCreateTemplateService::createDirectory");
    ...
    public function getTemplateTranslationPath($serviceName): string
    {
        return sprintf('%s/var/translation/%s', $this->container->getParameter('kernel.project_dir'), $serviceName);
    }

    public function readTemplateTranslationPath($directoryPath): array
    {
        $translations = [];
